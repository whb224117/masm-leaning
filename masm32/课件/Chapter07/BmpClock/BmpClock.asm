...
_CalcX		proc	_dwDegree,_dwRadius
		...
		fild	dwRadius			;将dwRadius压入FPU寄存器堆栈
		fild	_dwDegree		;将_dwDegree压入FPU寄存器堆栈
		fldpi						;将pi 即把3.1415926压栈 即赋值给ST(0)
		fmul						;角度*Pi			fmul 即 st(0) = st(0) * st(1)
		fild	dwPara180
		fdivp	st(1),st	;角度*Pi/180		FDIVP st(i),st      st(i) = st(0) /st(i)，然后执行一次出栈操作
		fsin			;Sin(角度*Pi/180)		st(0) = sin( st(0) )
		fild	_dwRadius
		fmul			;半径*Sin(角度*Pi/180)
		fadd			;X+半径*Sin(角度*Pi/180)		st(0) = st(0) + st(1)
		fistp	@dwReturn	;FISTP dest       dest = st(0) 然后再执行一次出栈操作
		mov	eax,@dwReturn
		ret
